...
const Skill = () => {
    ...
    const handleChange = (id, key, value) => {
        dispatch({ type: 'UPDATE_SKILL', id: id, key: key, value: value });
    }
    ...
    const handleClick = (action, id) => {
        switch (action) {
            case 'DELETE_SKILL':
                ...
                break;
            case 'ADD_SKILL':
                dispatch({ type: action, id: nextID++, skillName: '' })
                break;
            default:
                console.log("Switch case does nothing!");
                break;
        }
    }
    ...
    const placeholderArray = ['ReactJS', 'NodeJS', 'PostgreSQL', 'Tailwind CSS', 'ThreeJS',
                                'ExpressJS'];

    const skillInputNodes = cvState.skills.map((skill) => (
        <div key={skill.id} className="flex items-center gap-3">
            <input className="bg-input-box-gray h-12 w-full rounded px-3 text-white" 
            placeholder={placeholderArray[skill.id] || "Skill"} name="skillName" value={skill.skillName}
            onChange={(e) => handleChange(skill.id, e.target.name, e.target.value)} />
            <button className="pr-3" onClick={(e) => handleClick("DELETE_SKILL", skill.id)}>
    ...
    return (
        ...
        bg-component-gray h-fit w-full rounded-xl">
            <div className="grid grid-cols-[8fr_2fr] items-center ">
                ...
                <button className="flex gap-2 bg-mint rounded-xl p-4 text-lg items-center" onClick={(e) => handleClick('ADD_SKILL')}>
